...
class CacheDataloader(DataLoader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.cache_all_images:
            ...
            if isinstance(self.dataset, DynamicDataset) and self.dataset.use_importance_sampling:
                ist_w = None
                # IST weights in evaluation can be useful to compare how well the model does
                # on dynamic content and is more representative than
                ist_w = self.dataset.compute_is(self.cached_collated_batch, self.device, offline=True)
                self.cached_collated_batch["ist_weights"] = ist_w
    ...
    def _get_batch_list(self):
        ...
        #  2. "randsteps": random steps, same for each camera
        #  3. "lowfps": Lower FPS by k.
        pick_mode = "normal"
        if isinstance(self.dataset, DynamicDataset):
            pick_mode = self.dataset.pick_mode

        total_size = len(self.dataset)
        to_sample = self.num_images_to_sample_from

        if total_size == to_sample:
            pick_mode = "normal"

        indices: List

        if pick_mode == "normal":
            indices = random.sample(range(total_size), k=to_sample)
        elif pick_mode == "randsteps" or pick_mode == "lowfps":
            times = list(set(self.dataset.cameras.times.squeeze().tolist()))

            picked_times: List
            if pick_mode == "randsteps":
                cam_ids = self.dataset.cameras.ids.squeeze().tolist()

                # Unique cameras:
                nb_unique_cams = len(set(cam_ids))
                steps_to_pick = int(to_sample / nb_unique_cams)  # floor
                # Always include start and end
                picked_times = [times[0], times[-1]]
                picked_times += random.sample(times[1:-1], k=(steps_to_pick-2))
            elif pick_mode == "lowfps":
                # to_sample should be at the desired framerate
                if total_size % to_sample != 0:
                    print(
                        "[Warning | lowfps]: Ratio of images to sample over total is not an integer. Rounding down. This may result in a lower framerate than desired."
                    )

                # Pick every k frames
                k = ceil(total_size / to_sample)
                picked_times = times[::k]
                if len(times) % k != 0:
                    print(
                        "[Warning | lowfps]: Duration not dividable by desired framerate ratio. Rounding down. This may result in a lower framerate than desired."
                    )
                    # Remove one element
                    picked_times = picked_times[:-1]

                if total_size % to_sample != 0 or len(times) % k != 0:
                    print(
                        f"[Warning | lowfps]: effective FPS downscale chosen: {len(times) / len(picked_times)}. Desired was about {(total_size / to_sample):.4f}."
                    )

            indices = []
            for idx in range(total_size):
                if self.dataset.cameras.times[idx].item() in picked_times:
                    indices.append(idx)

            # Left to pick
            left_to_pick = to_sample - len(indices)
            if left_to_pick > 0:
                # print(f"[INFO]: Picking {left_to_pick} more images ({pick_mode})")
                range_wo_picked = [x for x in range(total_size) if x not in indices]
                indices += random.sample(range_wo_picked, k=left_to_pick)
        else:
            raise ValueError("Unknown pick_mode: " + pick_mode)

        # if isinstance(self.dataset, DynamicDataset):
        #    print(f"Picked {len(indices)} images with mode {pick_mode}.")

        if len(indices) != to_sample:
            raise Error("Not enough images to sample from.")
    ...
    def _get_collated_batch(self):
        ...
        # TODO exclude depth image
        collated_batch = get_dict_to_torch(collated_batch, device=self.device, exclude=["image", "depth_image"])
    ...
    def __iter__(self):
        while True:
            if self.cache_all_images:
                ...
                # Pre-compute importance sampling weights
                if isinstance(self.dataset, DynamicDataset) and self.dataset.use_importance_sampling:
                    if self.iter_step + self.num_times_to_repeat_images >= self.dataset.iters_to_start_ist:
                        # Also perform on evaluation dataset (was not the case beforehand)
                        ist_w = self.dataset.compute_is(collated_batch, self.device)
                        collated_batch["ist_weights"] = ist_w
                ...
                self.num_repeated += 1
            self.iter_step += 1
            collated_batch["iter_steps"] = self.iter_step
