...
class TemporalGridEncoder(nn.Module):
    ...
    def __init__(
        ...
        # allocate parameters
        offsets = []  # Contains the offset in the total hashmap for the start of each level.
        offset = 0  # Total number of parameters for all scales combined.
        ...
        for i in range(num_levels):
            ...
            offsets.append(offset)  # Append before adding to have the start (and not end) of each level.
            offset += params_in_level
        offsets.append(offset)  # Append the end of the last level.
        # offsets is therefore:
        # [start_level0, start_level1, ..., start_levelN, end_levelN]
        offsets = torch.from_numpy(np.array(offsets, dtype=np.int32))
        self.register_buffer("offsets", offsets)
        self.n_params = offsets[-1] * level_dim  # Total number: rows in hashmap * channels per row.
    ...
    def init_parameters(self) -> None:
        ...
        for _ in range(1, temporal_grid_rows - 1):
            ...
            new_index = torch.as_tensor(new_index, dtype=torch.long)  # Size 3
    ...
    def get_temporal_index(self, time: TensorType["bs"]) -> TensorType["bs", "temporal_index_dim"]:
        ...
        row_idx = row_idx_value.long()  # floored!
    ...
    def get_temporal_tv_loss(self) -> TensorType[()]:
        ...
        # temp_dim : temporal resolution (eg 64)
        # level_dim : features per level (eg 2)
        #

        # index_list : (temp_dim - 1) x (level_dim) [eg 63 x 3]
        # we take a random row of index_list
        # index_list eg : [0, 2, 1], [1, 3, 2], [2, 4, 3], ... [62, 64, 63]
        # Avec 4 features par level : [0, 4, 1, 2, 3], [1, 5, 4, 2, 3], [2, 6, 4, 5, 3], ... [62, 66, 64, 65, 63]
        # 1re élém : indice dans embeddings de la 1re feature de la temporal dim actuelle
        # 2e élém : indice dans embeddings de la 1re feature de la temporal dim suivante
        # 3e élém : indice de la feature en commun !
        # En effet, on a [0, 1] puis [2, 1] (pour que 1 soit tjrs en 2e position par cohérence)
        # aussi par exemple (0, 1, 2, 3), puis (4, 1, 2, 3), (4, 5, 2, 3), etc c'est 0 et 4.
        row_idx = torch.randint(0, len(self.index_list), [1]).item()
        # On récupère les indices des features dans l'embedding.
        feat_idx = self.index_list[row_idx]
        # L'embedding est de taille (nbr_total_features_explicites) x (level_dim + 1)
        # On compare donc la feature pas en commun à deux temps contigus.
