...
class Cameras(TensorDataclass):
    ...
    Args:
        ...
        ids: Unique ids for each camera. Allows to take advantage of multi-view setups.
    """
    ...
    ids: Optional[TensorType["num_cameras", 1]]
    ...
    def __init__(
        ...
        ids: Optional[TensorType["num_cameras"]] = None,
    ) -> None:
        ...
        self.ids = self._init_get_ids(ids)
    ...
    def _init_get_ids(self, ids: Union[None, torch.Tensor]) -> Union[None, torch.Tensor]:
        if ids is None:
            ids = None
        elif isinstance(ids, torch.Tensor):
            if ids.ndim == 0 or ids.shape[-1] != 1:
                ids = ids.unsqueeze(-1).to(self.device).to(torch.uint8)
        else:
            raise ValueError(f"times must be None or a tensor, got {type(ids)}")
