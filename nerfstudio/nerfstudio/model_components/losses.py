...
def interlevel_loss(weights_list, ray_samples_list):
    ...
    """
    # Detach to only affect proposal network(s)
...
def compute_plane_tv_old(t):
    ...
    """
    # 1, feature_dim, plane_res1, plane_res2
    batch_size, c, h, w = t.shape
    count_h = batch_size * c * (h - 1) * w
    count_w = batch_size * c * h * (w - 1)
    # Over h (rows) it's 2nd, 3rd, etc column - 1st, 2nd, etc column
    print((torch.square(t[..., 1:, :] - t[..., : h - 1, :])).mean())
    print((torch.square(t[..., 1:, :] - t[..., : h - 1, :])).sum() / count_h)
    print()
    h_tv = torch.square(t[..., 1:, :] - t[..., : h - 1, :]).sum()
    # Over w (cols) it's 2nd, 3rd, etc column - 1st, 2nd, etc column
