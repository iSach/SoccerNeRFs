...
class MedianRGBRenderer(nn.Module):
    """Render RGB value at position where the accumulated weight reaches 0.5.
    ...
    def combine_rgb(
        ...
        """Composite samples along ray and calculate RGB."""
        # Take RGB where accumulated weight reaches 0.5
        ...
        split = torch.ones((*weights.shape[:-2], 1), device=weights.device) * 0.5  # [..., 1]
        median_index = torch.searchsorted(cumulative_weights, split, side="left")  # [..., 1]
        median_index = torch.clamp(median_index, 0, weights.shape[-2] - 1)  # [..., 1]
        median_index = median_index.unsqueeze(dim=2).expand(-1, -1, 3)  # [..., 1, 3]
        median_rgb = torch.gather(rgb, dim=-2, index=median_index)  # [..., 1, 3]
...
class DecompositionRenderer(nn.Module):
    """Calculate normals along the ray."""

    @classmethod
    def forward(
        cls,
        probs: TensorType["bs":..., "num_samples", 3],
        weights: TensorType["bs":..., "num_samples", 1],
        ray_indices: Optional[TensorType["num_samples"]] = None,
        num_rays: Optional[int] = None,
    ) -> TensorType["bs":..., 3]:
        ...
        Args:
            probs: Probs for each sample.
            weights: Weights of each sample.
        """
        if ray_indices is not None and num_rays is not None:
            # Necessary for packed samples from volumetric ray sampler
            comp_probs = nerfacc.accumulate_along_rays(weights, ray_indices, probs, num_rays)
        else:
            comp_probs = torch.sum(weights * probs, dim=-2)
        return comp_probs
